...
if dust:
    plot_path = '/scratch/ntf229/RT_fit/plots/beaut_color_plots/'
elif noClumps:
    plot_path = '/scratch/ntf229/RT_fit/plots/beaut_color_plots_noClumps/'
elif partialDust:
    plot_path = '/scratch/ntf229/RT_fit/plots/beaut_color_plots_partialDust/'
else:
    plot_path = '/scratch/ntf229/RT_fit/plots/beaut_color_plots_nodust/'
...
if dust:
    ...
    SKIRT_path1 = '/scratch/ntf229/RT_fit/resources/SKIRT_noClumps/'
    SKIRT_path2 = '/numPhotons1e9/inc0/dust/dustFraction'
    SKIRT_path3 = '/maxTemp16000/'
elif partialDust:
    SKIRT_path1 = '/scratch/ntf229/RT_fit/resources/SKIRT_partialDust/'
    SKIRT_path2 = '/maxLevel13/wavelengths601/numPhotons1e9/inc0/dust/dustFraction'
    SKIRT_path3 = '/maxTemp16000/'
else:
    SKIRT_path1 = '/scratch/ntf229/RT_fit/resources/SKIRT/'
    SKIRT_path2 = '/maxLevel13/wavelengths601/numPhotons1e9/inc0/nodust/'
...
bands_short = ["FUV", "NUV", "u", "g", "r", "i", "z", "wise1", "wise2", "wise3", "wise4", "irac1", 
        "irac2", "irac3", "irac4", "mips24", "mips70", "mips160", "pacs100", "spire250", "spire500"]
...
ratio_indicies = [[0,6,20,6], [0,6,18,6], [0,6,10,6], [0,6,8,6]]
...
for i in range(len(galaxies)):
    for j in range(len(dustFraction)):
        ...
        if any([dust, partialDust, noClumps]):
        ...
        mags = observate.getSED(wave, f_lambda_cgs, filterlist=filterlist, linear_flux=False) # in AB Magnitudes
        jy = 1e26 * 10**(-1*(mags+48.6)/2.5) # convert from AB mags to Janskys (same order as bands)
        #maggies = observate.getSED(wave, f_lambda_cgs, filterlist=filterlist, linear_flux=True) # in maggies
        #jy = maggies * 3631 # convert from maggies to Janskys (same order as bands)
...
for i in range(len(ratio_indicies)): # number of plots to make 
    ...
    for j in range(len(dustFraction)):
        ...
        plt.scatter(SKIRT_flux[:,j,ratio_indicies[i][0]] / SKIRT_flux[:,j,ratio_indicies[i][1]], 
                            SKIRT_flux[:,j,ratio_indicies[i][2]] / SKIRT_flux[:,j,ratio_indicies[i][3]], marker='o', s=20, 
                            label='SKIRT dust-to-metal ratio '+dustFraction[j],zorder=10,alpha=0.7, c='blue')
    ...
    plt.xlabel(bands_short[ratio_indicies[i][0]]+' / '+bands_short[ratio_indicies[i][1]], fontsize=16)
    plt.ylabel(bands_short[ratio_indicies[i][2]]+' / '+bands_short[ratio_indicies[i][3]], fontsize=16)
    plt.xscale('log')
    plt.yscale('log')
