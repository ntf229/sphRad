...
axisRatio_color_indices = [[0,6], [1,6], [2,6], [3,6], [4,6], [5,6]]
...
for i in range(len(galaxies)):
    ...
    plt.close()
    # Axis Ratios
    print('starting axis ratio calculation')
...
# Color-color and axisRatioColor Plots
...
for i in range(len(ratio_indicies)): # number of plots to make 
    ...
    # axis ratio vs. color
    plt.figure(figsize=(10,8))
    yerr = np.absolute(dp_flux[:,axisRatio_color_indices[i][0]] / dp_flux[:,axisRatio_color_indices[i][1]]) * np.sqrt( 
        			  (dp_err[:,axisRatio_color_indices[i][0]]/dp_flux[:,axisRatio_color_indices[i][0]])**2 + 
        			  (dp_err[:,axisRatio_color_indices[i][1]]/dp_flux[:,axisRatio_color_indices[i][1]])**2 )
    colors = np.empty(dp_num,dtype=str)
    for d in range(dp_num):
        bad = False
        xuplims = False
        ...
        if bad:
            ...
            plt.errorbar(dp_axisRatio[d], 
                         dp_flux[d,axisRatio_color_indices[i][0]] / dp_flux[d,axisRatio_color_indices[i][1]], 
                         xerr=0, yerr=yerr[d], elinewidth=0.2, marker='o',
                         markersize=5, linewidth=0, color=colors[d], zorder=0, alpha=0.3,
                         xuplims=xuplims, xlolims=xlolims, uplims=uplims, lolims=lolims)
    plt.scatter(SKIRT_axisRatio, 
                SKIRT_flux[:,axisRatio_color_indices[i][0]] / SKIRT_flux[:,axisRatio_color_indices[i][1]], 
                marker='o', s=20, zorder=10,alpha=0.7, c='blue')
    plt.xlabel('Axis Ratio', fontsize=16)
    plt.ylabel(band_names[axisRatio_color_indices[i][0]]+' / '+band_names[axisRatio_color_indices[i][1]], fontsize=16)
    #plt.xscale('log')
    plt.yscale('log')
    plt.xticks(fontsize=16)
    plt.yticks(fontsize=16)
    #plt.xlim((xlims[i][0], xlims[i][1]))
    #plt.ylim((ylims[i][0], ylims[i][1]))
    plt.savefig(plotPath+'axisRatioColorPlots/'+axisRatio_plot_names[i]+'.png',dpi=600)
    plt.close()
print('done')
